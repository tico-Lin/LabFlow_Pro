...
fn update_file_metadata(
    ...
    let node_uuid =
        Uuid::parse_str(&node_id).map_err(|err| format!("invalid node id: {err}"))?;
    ...
    let mut properties: BTreeMap<String, String> = existing_node.payload.properties.clone();
    let tags_json = serde_json::to_string(&tags)
        .map_err(|err| format!("failed to serialize tags: {err}"))?;
    properties.insert("tags".to_string(), tags_json);
    properties.insert("remark".to_string(), remark);

    let mut payload = NodePayload::new(existing_node.payload.label.clone());
    payload.properties = properties;
    payload.content = existing_node.payload.content.clone();
    ...
    emit_graph_updated(&window, "metadata_updated", vec![op_id])?;
    Ok(())
}
