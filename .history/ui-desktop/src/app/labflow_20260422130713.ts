...
export function parseAnalysisModules(payload: string): AnalysisModule[] {
  const parsed = JSON.parse(payload) as unknown;
  if (!Array.isArray(parsed)) {
    throw new Error("Analysis module catalog must be a JSON array");
  }

  return parsed.map((entry) => {
    if (!entry || typeof entry !== "object") {
      throw new Error("Invalid analysis module payload");
    }

    const module = entry as Partial<AnalysisModule>;
    if (
      typeof module.id !== "string" ||
      typeof module.name !== "string" ||
      typeof module.description !== "string" ||
      !Array.isArray(module.supportedFormats) ||
      !Array.isArray(module.parameters)
    ) {
      throw new Error("Analysis module is missing required fields");
    }
    ...
    return {
      ...
      supportedFormats: module.supportedFormats.map((format) => {
        if (typeof format !== "string") {
          throw new Error(`Invalid supported format for module ${module.id}`);
        }
...
export function buildAnalysisModuleFormState(
  module: AnalysisModule | undefined
