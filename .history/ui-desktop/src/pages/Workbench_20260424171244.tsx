...
const DATA_EXTENSIONS = new Set(["csv", "txt", "dat", "tsv", "asc"]);
...
function buildFileSpreadsheetData(xValues: number[], yValues: number[]): SpreadsheetGridData {
  ...
  for (let i = 0; i < pointCount; i++) {
    cells[`${i + 2}:1`] = xValues[i];
    cells[`${i + 2}:2`] = yValues[i];
  }
  return { rows: Math.max(20, pointCount + 2), cols: 3, cells };
}
...
export default function Workbench({
  ...
  useEffect(() => {
    if (!targetNodeId || !graph) {
      setFileViewMode("none");
      return;
    }

    const node = graph.nodes.find((n) => n.id === targetNodeId);
    if (!node || normalizeGraphNodeType(node.properties) !== "file") {
      setFileViewMode("none");
      return;
    }

    const ext = (node.properties.extension ?? "").toLowerCase();
    const hash = node.properties.hash ?? "";

    if (PDF_EXTENSIONS.has(ext)) {
      setFileViewMode("pdf");
      setFileHash(hash);
      setFileChartData(null);
      setFileSpreadsheetData(null);
    ...
    if (DATA_EXTENSIONS.has(ext)) {
      ...
      const loadData = async () => {
        try {
          const raw = await invoke<number[]>("read_blob_bytes", { hash });
          if (cancelled) {
            return;
          }
          ...
          if (!cancelled) {
            setFileLoadError(err instanceof Error ? err.message : String(err));
    ...
    // All other file types — just show metadata, keep existing chart/spreadsheet props
    setFileViewMode("none");
    setFileHash(null);
  }, [targetNodeId, graph]);
      ...
      <div className="workbench-content-grid">
        <div className="workbench-main-column">
          {fileViewMode === "pdf" && fileHash ? (
            ...
            <div className="workbench-file-status is-error" role="alert">
              <p>{fileLoadError}</p>
            </div>
          ) : (
