...
import type { GraphStateSnapshot } from "../app/labflow";
...
function parseFileNode(node: any): {
  name: string;
  hash: string;
  extension: string;
  sizeBytes: number;
  ...
  const hash = typeof node.blob_hash === "string"
    ? node.blob_hash
    : (typeof node.hash === "string" ? node.hash : "");
  const extension = typeof node.extension === "string" ? node.extension : "";
  const sizeRaw = Number(node.size ?? 0);

  let tags: string[] = [];
  if (Array.isArray(node.tags)) {
    tags = node.tags.filter((item: unknown): item is string => typeof item === "string");
  } else if (typeof node.tags === "string" && node.tags.trim()) {
    try {
      const parsed = JSON.parse(node.tags) as unknown;
  ...
  const remark = typeof node.remark === "string" ? node.remark : "";
  const alignmentStatus = typeof node.alignment_status === "string" ? node.alignment_status : "";
  const isAlignedRaw = node.is_aligned;
  const isAligning = isAlignedRaw === 0 || isAlignedRaw === false || isAlignedRaw === "0";
...
export default function AssetHubView({ graph, onRefresh }: AssetHubViewProps) {
  ...
  useEffect(() => {
    void fetchFiles().catch((fetchError) => {
            ...
            {files.length === 0 ? (
              ...
              <section className="asset-grid" aria-label={t("assetHub.gridAriaLabel")}>
                {files.map((node) => {
                  ...
                  const isOrphaned = file.alignmentStatus === "ORPHANED";
                  const disableNavigation = isOrphaned || file.isAligning;

                  return (
                    <article
                      key={node.id}
                      className={`app-surface asset-file-card ${disableNavigation ? "asset-file-card--disabled" : "is-clickable"}`}
                      ...
                      onKeyDown={(event) => {
                        if (!disableNavigation && (event.key === "Enter" || event.key === " ")) {
                    ...
                    >
                      <div className="asset-file-card-head">
                        ...
                        <div className="asset-file-meta">
                          ...
                          <p>.{iconExtension}</p>
                          {file.isAligning ? (
                            <span className="status-badge-aligning" aria-live="polite">🔄 對齊中 (Aligning...)</span>
                          ) : null}
                          {isOrphaned ? (
                            <span className="status-badge-error" aria-live="polite">⚠️ 檔案遺失 (Orphaned)</span>
                          ) : null}
                        </div>
                        <button
                          type="button"
                          className="icon-button asset-file-edit-btn"
                          aria-label={t("assetHub.fileCard.edit")}
                          onClick={(e) => {
                            e.stopPropagation();
                            setEditTarget({ nodeId: node.id, tags: file.tags, remark: file.remark });
                          }}
                        >
                          <Pencil size={14} />
                        </button>
                      ...
                      {file.remark ? (
                        <p className="asset-file-remark">
                          {file.remark.length > 30 ? `${file.remark.slice(0, 30)}…` : file.remark}
                        </p>
                      ) : null}
