...
export default function Workbench({
  ...
  const selectedFileNode = useMemo(() => {
    ...
    return graph.nodes.find((node) => node.id === targetNodeId) ?? null;
  }, [graph, targetNodeId]);
  ...
  const selectedFileExtension = useMemo(() => {
    ...
    const extension = selectedFileNode.properties.extension;
    return typeof extension === "string" ? extension.toLowerCase() : "";
  }, [selectedFileNode]);

  const selectedFileHash = useMemo(() => {
    if (!selectedFileNode) {
      return "";
    }

    const hash = selectedFileNode.properties.hash;
    return typeof hash === "string" ? hash : "";
  }, [selectedFileNode]);

  const isPdfFileView =
    !!selectedFileNode &&
    normalizeGraphNodeType(selectedFileNode.properties) === "file" &&
            ...
            {isPdfFileView ? (
              <div key="spreadsheet" style={WORKBENCH_CARD_STYLE}>
                ...
                <div className="workbench-grid-item-body">
                  <PdfViewer hash={selectedFileHash} />
                </div>
              </div>
            ) : (
              <>
