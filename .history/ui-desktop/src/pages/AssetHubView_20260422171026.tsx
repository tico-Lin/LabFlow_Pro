...
function formatFileSize(bytes: number, t: (key: string, values?: Record<string, unknown>) => string): string {
  if (bytes <= 0) {
    return t("assetHub.fileCard.zeroSize");
  }
              ...
              <article key={node.id} className="app-surface asset-file-card">
                ...
                <dl className="asset-file-stats">
                  <div>
                    ...
                    <dd>{formatFileSize(file.sizeBytes, t)}</dd>
                  </div>
                  <div>
