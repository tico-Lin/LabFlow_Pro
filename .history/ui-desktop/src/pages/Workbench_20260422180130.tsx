...
const WORKBENCH_LAYOUT_STORAGE_KEY = "workbench-layout";
const DEFAULT_WORKBENCH_LAYOUT: Layout[] = [
  { i: "spreadsheet", x: 0, y: 0, w: 12, h: 4 },
  { i: "chart", x: 0, y: 4, w: 12, h: 6 }
];
...
export default function Workbench({
  ...
  const [workbenchLayout, setWorkbenchLayout] = useState<Layout[]>(() => {
    ...
    try {
      ...
      const normalizedLayout = parsedLayout.filter((entry): entry is Layout => {
        if (!entry || typeof entry !== "object") {
          return false;
        }

        const candidate = entry as Partial<Layout>;
        return (
  ...
  const workbenchLayouts = useMemo(
    () => ({
      lg: workbenchLayout,
      md: workbenchLayout,
      sm: workbenchLayout,
