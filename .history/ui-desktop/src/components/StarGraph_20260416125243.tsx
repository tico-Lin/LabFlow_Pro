...
export function StarGraph({
  ...
  useEffect(() => {
    ...
    const renderFrame = (timestamp: number) => {
      ...
      for (const node of simNodes) {
        ...
        context.lineWidth = draggingNodeId === node.id || linkingNodeId === node.id ? 3 : 2;
  ...
  }, [draggingNodeId, linkingNodeId, viewport]);
    ...
    <div className="star-graph-shell" ref={containerRef}>
      <div className="star-graph-header">
        <div>
          ...
          <p>雙擊空白建立筆記，雙擊節點開啟內容，按住 Alt 從 A 拖到 B 可手動建立血緣連線。</p>
        </div>
