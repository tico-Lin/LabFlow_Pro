...
function getPointerPosition(event: React.MouseEvent<HTMLCanvasElement>, canvas: HTMLCanvasElement) {
...
export function StarGraph({ nodes, edges, height = DEFAULT_HEIGHT }: StarGraphProps) {
  ...
  const nodeCountLabel = useMemo(() => `${nodes.length} nodes`, [nodes.length]);
  ...
  useEffect(() => {
    ...
    const renderFrame = (timestamp: number) => {
      ...
      for (const node of simNodes) {
        ...
        context.lineWidth = draggingNodeId === node.id ? 3 : 2;
        ...
        context.fillText(node.label, node.x, node.y - 1, 96);

        context.fillStyle = "rgba(226, 232, 240, 0.8)";
        context.font = "10px 'Segoe UI', sans-serif";
        context.fillText(node.type, node.x, node.y + 14, 90);
      }
  ...
  const handlePointerDown = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const targetNode = [...nodesRef.current]
      .reverse()
      .find((node) => Math.hypot(pointer.x - node.x, pointer.y - node.y) <= node.radius + 6);
    ...
    <div className="star-graph-shell" ref={containerRef}>
      <div className="star-graph-header">
        <div>
          ...
          <p>純 Canvas 力導向模擬，支援拖拉節點與高解析度繪製。</p>
        </div>
      ...
      <canvas
        ...
        style={{ cursor: draggingNodeId ? "grabbing" : "grab" }}
      />
    </div>
  );
}
