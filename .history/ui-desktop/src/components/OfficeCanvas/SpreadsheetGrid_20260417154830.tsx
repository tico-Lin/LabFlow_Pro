...
export type SpreadsheetGridData = {
  rows: number;
  cols: number;
  cells: Record<string, CellValue> | CellValue[];
};
...
function getCellStore(data: SpreadsheetGridData): Record<string, CellValue> {
  return Array.isArray(data.cells) ? {} : data.cells;
}

function ensureCellStore(data: SpreadsheetGridData): Record<string, CellValue> {
...
function drawCell(
  ...
  if (row === 0 && col === 0) {
    ...
    const value = cells[cellKey(row, col)] ?? null;
    ctx.fillStyle = theme.canvasText;
    ctx.font = "12px Segoe UI";
    ctx.textBaseline = "middle";
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key.length === 1 && /^[a-zA-Z0-9]$/.test(e.key)) {
        const cells = ensureCellStore(dataRef.current);
        let prev = cells[key] ?? "";
