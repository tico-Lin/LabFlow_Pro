...
export default function SpreadsheetGrid({
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      const measuredWidth = shell.clientWidth;
      const measuredHeight = shell.clientHeight;
      const nextWidth = resizable ? Math.max(360, measuredWidth - 2) : viewportWidth;
      const nextHeight = resizable ? Math.max(220, measuredHeight - 54) : viewportHeight;
  ...
  return (
    <div
      ref={shellRef}
      className={`spreadsheet-grid-shell${resizable ? " is-resizable" : ""}`}
