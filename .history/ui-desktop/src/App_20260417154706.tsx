...
import { createDemoSpreadsheetData, type CellPointer, type CellValue, type SpreadsheetGridData } from "./components/OfficeCanvas/SpreadsheetGrid";
...
function getSpreadsheetCells(cells: SpreadsheetGridData["cells"]): Record<string, CellValue> {
...
export default function App() {
  ...
  useEffect(() => {
    setSpreadsheetData((prev) => {
      ...
      const cells = getSpreadsheetCells(prev.cells);
      ...
      const isDemoGrid =
        typeof cells["1:1"] === "string" &&
        typeof cells["1:2"] === "string" &&
        typeof cells["1:3"] === "string" &&
        knownDemoHeaders.has(String(cells["1:1"])) &&
        knownDemoHeaders.has(String(cells["1:2"])) &&
        knownDemoHeaders.has(String(cells["1:3"]));
  ...
  const analyzePeak = useCallback(async () => {
    ...
    const cells = getSpreadsheetCells(spreadsheetData.cells);

    for (let row = 2; row <= spreadsheetData.rows; row += 1) {
      const voltage = cells[`${row}:1`];
      const current = cells[`${row}:2`];
