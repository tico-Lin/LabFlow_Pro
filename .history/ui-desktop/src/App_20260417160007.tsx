...
import { Activity, Database, FilePenLine, Languages, Network, RefreshCw, Settings, type LucideIcon } from "lucide-react";
...
type SidebarItemKey = "dashboard" | "notes" | "workbench" | "graph";
...
export default function App() {
  ...
  const emptySpreadsheetData = useMemo<SpreadsheetGridData>(() => ({ rows: 24, cols: 12, cells: {} }), []);
  const [spreadsheetData, setSpreadsheetData] = useState<SpreadsheetGridData>(emptySpreadsheetData);
  ...
  const reportDataParseWarning = useCallback((error: unknown) => {
    console.error(error);
    setChartData([]);
    setSpreadsheetData(emptySpreadsheetData);
    setPeakIndex(undefined);
    setPeakRow(null);
    setFocusedRow(null);
    setMetadata(null);
    setInstrumentFormat(t("common.unknown"));
    setError(null);
  }, [emptySpreadsheetData, t]);
  ...
  const displaySpreadsheetData = useMemo(
    () => (chartData.length > 0 ? spreadsheetData : emptySpreadsheetData),
    [chartData.length, emptySpreadsheetData, spreadsheetData]
  );
    ...
    const setupListener = async () => {
      activeUnlisten = await listen<GraphUpdatedPayload>("graph-updated", (event) => {
        try {
          ...
          if (resultPayload) {
            applyInstrumentContent(payload, t("common.unknown"));
            void fetchState();
            return;
          }
          ...
          <Routes>
            ...
            <Route
              ...
              element={
                <Workbench
                  theme={theme}
                  spreadsheetData={displaySpreadsheetData}
                  revision={revision}
                  peakRow={peakRow}
                  ...
                  emptyNotice={chartData.length === 0 ? t("workbench.emptyNotice") : null}
