...
/// 使用 PyO3 調用 Python 分析模組，尋找最大電流峰值
pub fn run_peak_analysis(voltages: Vec<f64>, currents: Vec<f64>) -> Option<(usize, f64, f64)> {
    Python::with_gil(|py| {
        // 將 Rust Vec 轉為 Python list
        let py_voltages = PyList::new_bound(py, &voltages);
        let py_currents = PyList::new_bound(py, &currents);

        // 直接用 py.eval 執行分析邏輯（避免 PYTHONPATH 問題）
        let code = r#"
def find_max_peak(voltages, currents):
...
"#;
        ...
        locals.set_item("voltages", py_voltages).ok()?;
        locals.set_item("currents", py_currents).ok()?;
        py.run_bound(code, None, Some(&locals)).ok()?;
        let result = locals.get_item("result").ok()??;
        let index: usize = result.get_item("index").ok()?.extract().ok()?;
        let voltage: f64 = result.get_item("voltage").ok()?.extract().ok()?;
        let current: f64 = result.get_item("current").ok()?.extract().ok()?;
        Some((index, voltage, current))
    })
}
