...
//! | `DeleteNode` vs any node write, same node          | Delete-Wins tombstone |
...
impl NodePayload {
    ...
    pub fn with_content(label: impl Into<String>, content: JsonValue) -> Self {
        Self {
            label: label.into(),
            properties: BTreeMap::new(),
    ...
    pub fn note(title: impl Into<String>, content: impl Into<String>) -> Self {
        ...
        let mut payload = Self::with_content(
            title.clone(),
            json!({
                "title": title,
                "content": content,
        ...
        payload.properties.insert("type".to_string(), "note".to_string());
        payload
    }
}
...
pub enum OpKind {
    ...
    /// Permanently tombstone a node and hide all incident edges.
    DeleteNode {
        node_id: NodeId,
    },
...
pub struct GraphState {
    ...
    /// Tombstone set for deleted nodes; grows monotonically (append-only).
...
impl GraphState {
    ...
    pub fn snapshot(&self) -> GraphSnapshot {
        ...
        let mut deleted_nodes: Vec<NodeId> = self.deleted_nodes.iter().copied().collect();
