...
impl ShadowGraph {
    ...
    /// Stage a `DeleteNode` operation in the delta buffer.
    pub fn delete_node(&mut self, node_id: NodeId) -> &Operation {
        let op = Operation::new(
...
mod tests {
    ...
    #[test]
    fn delete_node_in_shadow_only_applies_after_commit() {
        let mut main = MainGraph::new(user_peer());
        let node_id = node(0xCAFE);
        main.insert_node(node_id, payload("Shadowed"));

        let mut shadow = main.create_shadow(agent_peer());
        shadow.delete_node(node_id);

        assert!(main.state().nodes.contains_key(&node_id));

        main.commit_to_main(shadow).unwrap();

        assert!(!main.state().nodes.contains_key(&node_id));
        assert!(main.state().deleted_nodes.contains(&node_id));
    }
