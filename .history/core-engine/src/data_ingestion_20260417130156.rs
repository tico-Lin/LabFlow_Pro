...
fn parse_numeric_pair(line: &str) -> Option<(f64, f64)> {
    let values = line
        ...
        .filter_map(|token| f64::from_str(token).ok())
        .take(2)
        .collect::<Vec<_>>();
...
pub fn ingest_ascii_data(ascii: &str, peer_id: PeerId) -> Vec<Operation> {
    ...
    let dataset_label = format!(
        "{} Dataset",
        instrument_format_name(detected_format).to_ascii_uppercase()
    );
    let mut payload = NodePayload::with_content(dataset_label, payload_json);
    ...
    payload
        .properties
        .insert("grid_anchor_row".to_string(), "2".to_string());
...
mod tests {
    ...
    fn ingest_ascii_data_auto_routes_and_roundtrips() {
        ...
        match &decoded[0].kind {
            OpKind::InsertNode { payload, .. } => {
                ...
                let payload_json = payload.content.clone().expect("payload content should exist");
    ...
    #[test]
    fn ingest_ascii_data_tolerates_crlf_headers_and_mixed_case_keywords() {
        let cv_input = "Instrument: Potentiostat\r\nSCAN RATE: 50 mV/s\r\nVoltage,Current\r\nHeader,Ignore Me\r\n -0.10 , 0.001 \r\nnot,a,number\r\n0.00,0.004\r\n0.10,0.009\r\n";
        let xrd_input = "# xRd EXPORT\r\nTITLE: Powder Scan\r\n2THETA,INTENSITY\r\ncomment,skip\r\n10.0,1200\r\n10.5,1215\r\n11.0,1198\r\n";
        let peer_id = Uuid::new_v4();
