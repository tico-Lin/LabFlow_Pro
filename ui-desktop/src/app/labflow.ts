...
export type AnalysisModule = PluginManifest;
...
function isAnalysisModuleParameterType(value: unknown): value is AnalysisModuleParameterType {
  return value === "number" || value === "string" || value === "boolean";
}

function parseAnalysisModuleParameter(value: unknown): AnalysisModuleParameter {
  if (!value || typeof value !== "object") {
    throw new Error("Invalid analysis module parameter payload");
  }

  const parameter = value as Partial<AnalysisModuleParameter>;
  if (typeof parameter.key !== "string" || typeof parameter.name !== "string") {
    throw new Error("Analysis module parameter is missing required string fields");
  }
  if (!isAnalysisModuleParameterType(parameter.type)) {
    throw new Error(`Unsupported parameter type for ${parameter.key}`);
  }
  ...
  if (!isValidDefaultValue) {
    throw new Error(`Invalid default value for ${parameter.key}`);
  }
...
export function parsePluginManifests(payload: unknown): PluginManifest[] {
  const parsed = typeof payload === "string" ? (JSON.parse(payload) as unknown) : payload;
  if (!Array.isArray(parsed)) {
    throw new Error("Plugin manifest catalog must be an array");
  }
  ...
  return parsed.map((entry) => {
    ...
    const supportedFormatsRaw = module.supportedFormats ?? module.supported_formats;
    const executeCmdRaw = module.executeCmd ?? module.execute_cmd;

    if (
      typeof module.id !== "string" ||
      typeof module.name !== "string" ||
      typeof module.description !== "string" ||
      typeof module.engine !== "string" ||
      !Array.isArray(supportedFormatsRaw) ||
      !Array.isArray(executeCmdRaw) ||
    ...
    return {
      ...
      executeCmd: executeCmdRaw.map((segment) => {
        if (typeof segment !== "string") {
          throw new Error(`Invalid execute command for plugin ${module.id}`);
        }

        return segment;
      }),
      supportedFormats: supportedFormatsRaw.map((format) => {
        if (typeof format !== "string") {
          throw new Error(`Invalid supported format for plugin ${module.id}`);
        }

        return format;
      }),
      parameters: module.parameters.map((parameter) => parseAnalysisModuleParameter(parameter))
    };
  });
}

export function parseAnalysisModules(payload: unknown): AnalysisModule[] {
  return parsePluginManifests(payload);
}

export function buildAnalysisModuleFormState(
  module: PluginManifest | undefined
  ...
  return Object.fromEntries(
    module.parameters.map((parameter) => [parameter.key, String(parameter.defaultValue)])
  );
}
...
export function parseNoteDocument(node: GraphNodeSnapshot, fallbackTitle: string): NoteDocument {
  ...
  const propertyTitle = typeof node.properties.title === "string" ? node.properties.title.trim() : "";
  const title =
    propertyTitle ||
    (typeof contentObject?.title === "string" && contentObject.title.trim() ? contentObject.title : node.label || fallbackTitle);
